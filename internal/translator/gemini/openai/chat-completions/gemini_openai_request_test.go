...
func TestConvertOpenAIRequestToGeminiImageGenerationTool(t *testing.T) {
	t.Run("tools with image_generation sets responseModalities without function declarations", func(t *testing.T) {
		...
		out := ConvertOpenAIRequestToGemini("gemini-2.0-flash", inputJSON, false)

		mods := gjson.GetBytes(out, "generationConfig.responseModalities").Array()
		if len(mods) != 2 {
		...
		if gjson.GetBytes(out, "tools").Exists() {
			t.Fatalf("tools should not be present when only image_generation is provided. Output: %s", out)
		}
		if gjson.GetBytes(out, "toolConfig").Exists() {
			t.Fatalf("toolConfig should not be present. Output: %s", out)
		}
	})
	...
	t.Run("tools with size mapping", func(t *testing.T) {
		...
		for _, tc := range tests {
			t.Run(tc.size, func(t *testing.T) {
				inputJSON := []byte(`{"model":"gemini-2.0-flash","messages":[{"role":"user","content":"Draw an image"}],"tools":[{"type":"image_generation","size":"` + tc.size + `"}]}`)
				out := ConvertOpenAIRequestToGemini("gemini-2.0-flash", inputJSON, false)
				if got := gjson.GetBytes(out, "generationConfig.imageConfig.aspectRatio").String(); got != tc.wantAR {
	...
	t.Run("tool_choice with type image_generation", func(t *testing.T) {
		...
		if got := gjson.GetBytes(out, "generationConfig.imageConfig.aspectRatio").String(); got != "4:3" {
	...
	t.Run("tools with both function and image_generation does not declare image_generation as function", func(t *testing.T) {
		...
		fnDecls := gjson.GetBytes(out, "tools.0.functionDeclarations").Array()
		if len(fnDecls) != 1 {
		...
		// tool_choice was image_generation, so toolConfig should NOT be set
		if gjson.GetBytes(out, "toolConfig").Exists() {
			t.Fatalf("toolConfig should not be set when tool_choice is image_generation. Output: %s", out)
		}
	})
}
